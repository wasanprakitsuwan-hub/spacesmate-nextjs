...
// Compute counts once for sidebar badges
const TYPE_COUNTS: Record<string, number> = { '': properties.length }
properties.forEach(p => { TYPE_COUNTS[p.propertyType] = (TYPE_COUNTS[p.propertyType] || 0) + 1 })
...
function SearchContent() {
  ...
  const results = useMemo(() => {
    ...
    return list
  }, [activeType, priceRangeIdx, sortBy])
                ...
                {TYPE_FILTERS.map(t => {
                  ...
                  return (
                    <button key={t.key} onClick={() => selectType(t.key)}
                      ...
                      <span>{t.label}</span>
                      <span className="mono" style={{ fontSize: 10.5, opacity: .7 }}>{TYPE_COUNTS[t.key] ?? 0}</span>
                    </button>
                  )
                })}
              </div>
