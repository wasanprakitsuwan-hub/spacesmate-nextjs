...
function RichEditor({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
  ...
  const btn: React.CSSProperties = { padding: '4px 8px', borderRadius: 6, border: '1px solid #eef0ef', background: '#f8fafc', color: '#475569', cursor: 'pointer', fontSize: 12, fontWeight: 600, lineHeight: 1 }
  return (
    <div style={{ border: '1px solid #eef0ef', borderRadius: 12, overflow: 'hidden' }}>
      <div style={{ display: 'flex', gap: 4, padding: '8px 10px', background: '#f8fafc', borderBottom: '1px solid #eef0ef', flexWrap: 'wrap' }}>
        <button type="button" style={btn} onMouseDown={e => { e.preventDefault(); exec('bold') }}><b>B</b></button>
        <button type="button" style={btn} onMouseDown={e => { e.preventDefault(); exec('italic') }}><i>I</i></button>
        <button type="button" style={btn} onMouseDown={e => { e.preventDefault(); exec('underline') }}><u>U</u></button>
